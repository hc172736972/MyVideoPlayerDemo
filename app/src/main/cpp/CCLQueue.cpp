...
CCLQueue::CCLQueue(CCLPlayStatus *playStatus) {
...
int CCLQueue::putAvpacket(AVPacket *avPacket) {
    pthread_mutex_lock(&mutexPacket);
...
int CCLQueue::getAvpacket(AVPacket *avPacket) {
    pthread_mutex_lock(&mutexPacket);
    while (cclPlayStatus != NULL && !cclPlayStatus->exit) {
...
int CCLQueue::noticeThread() {
    pthread_cond_signal(&condFrame);
    pthread_cond_signal(&condPacket);
    return 0;
}

void CCLQueue::release() {
    if (LOG_IS_OPEN) {
        LOGD("CCLQueue::release");
    }
    ...
    if (LOG_IS_OPEN) {
        LOGD("CCLQueue::release success");
    }

}

int CCLQueue::putAvframe(AVFrame *pFrame) {
...
int CCLQueue::getAvframe(AVFrame *pFrame) {
